...
| App servers | Vite 6.4.3 **dev** `http://127.0.0.1:5173` and **preview** `http://127.0.0.1:4173` / `4174` of `npm run build` |
...
- `e2e/shop.spec.ts` — help-accepted through shop success with physical actions. Screenshots: `shelf-record.png`, `calculator.png`, `notice.png`, `success.png`. Overlay bounds at 1366 and 1920: `overlays.png`.
...
Shop cards look like paper records (not quizzes). Calculator result 17. Posted notice shows ١٧ and a source-backed fact without presenting ٢٠ / تمر ١٨ / ماء نفد as true. After success the shopkeeper thanks the player and the HUD lead is the repair parcel; the library inner door stays locked.
