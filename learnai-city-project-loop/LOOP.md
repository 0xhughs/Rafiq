...
## Execution adapter for this prepared pack
This is a portable planning pack, not an installed background service. Creating it does not run it. After explicit execution authorization, record the actual coordinator identity and available spawn, send, wait/status and stop tools in BUILD.md before any dispatch. Use separate Builder and Reviewer agents; prefer a fresh Reviewer context containing the contract and candidate evidence without Builder reasoning. Keep one active worker per checkout and confirm completion before transferring ownership.

If independent review tools are unavailable, preserve state and report the limitation; never simulate independent approval by changing personas. Do not create separate user-visible tasks, automations or runtime settings to emulate missing delegation. A stopped foreground turn is not continuing background work.

Configure exact snapshot and contract capture/recheck commands for the implementation checkout before first dispatch. Include the curriculum translation and project decisions in SLICES.md, plus any adopted normative reference constraints, in contract coverage. REFERENCES.md supplies provenance, not a second execution authority. Never mistake the source ZIP hash or a reference repository commit for the implementation candidate identity.
